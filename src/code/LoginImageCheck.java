 ...
 * @author user
 */
public class LoginImageCheck extends javax.swing.JFrame {
     ...
     */
    public LoginImageCheck() throws IOException {
        initComponents();
    ...
    public LoginImageCheck(String name) throws IOException {
        initComponents();
        user_name = name;
        
        pattern_for_confirm = retrievePattern(name);
        System.out.println(pattern_for_confirm);
        loadImage();
        getContentPane().remove(getContentPane);
        revalidate();
        try{
    ...
    public LoginImageCheck(String name, List<Integer> pattern_to_confirm, int attempt) throws IOException {
        initComponents();
        user_name = name;
        attempts = attempt;
...
//            getContentPane().add(button_panel);
    ...
    private void initComponents() {
        ...
        getContentPane().add(accept, new org.netbeans.lib.awtextra.AbsoluteConstraints(150, 400, 129, 63));
        ...
        if(pattern_for_confirm.size() > 3 && attempts > 1){
            try{
                if(pattern.equals(pattern_for_confirm)){
                        JOptionPane.showMessageDialog(null, "Success", "Logged In Succesfully" , JOptionPane.WARNING_MESSAGE);
                        System.exit(0);
                }else{
                    attempts--;
                    ...
                    LoginImageCheck recalling = new LoginImageCheck(user_name, pattern_for_confirm, attempts);
    ...
    public static void main(String args[]) {
        ...
        try {
            ...
            java.util.logging.Logger.getLogger(LoginImageCheck.class.getName()).log(java.util.logging.Level.SEVERE, null, ex);
        ...
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
                try {
                    new LoginImageCheck().setVisible(true);
                } catch (IOException ex) {
                    Logger.getLogger(LoginImageCheck.class.getName()).log(Level.SEVERE, null, ex);
                }
            }
        });
    }
